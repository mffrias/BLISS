...
public static ClassLoader efficientClassLoader = null;
	...
	public static HashMap<String,Class<?>> classesUnderAnalysis = new HashMap<String, Class<?>>();
	
	public static Integer rootIndexHackTSE = null;
	
	public static Integer getRootIndex(SymbolicInputHeap sih){
		if (rootIndexHackTSE != null){
			return rootIndexHackTSE;
		} else {
			rootIndexHackTSE = sih.getRootIndexFromAnySymbolicIntegerInHeap();
			return rootIndexHackTSE;
		}
	}
	...
	public static Class<?> loadClassUnderAnalysis(String className){
		...
		if (!classesUnderAnalysis.containsKey(className)){
			try {
				URL classURL = null;
				try{
					String workDirectory = System.getProperty("user.dir");
					classURL = new URL("file://" + workDirectory + "/build/examples/");
				} catch (MalformedURLException e) {}
				
				URLClassLoader cl;
				
				if (efficientClassLoader == null){

					URL[] classURLs = new URL[]{classURL};

					cl = new URLClassLoader(classURLs);
					
					efficientClassLoader = cl;
				}	
				
				cl = (URLClassLoader) efficientClassLoader;

				Class<?> loadedClass = cl.loadClass(className);
				classesUnderAnalysis.put(className, loadedClass);
...
//				e.printStackTrace();
		...
		if (classesUnderAnalysis.containsKey(className)){
			return classesUnderAnalysis.get(className);
		} else
