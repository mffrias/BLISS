...
public class SymbolicInstructionFactory extends gov.nasa.jpf.jvm.bytecode.InstructionFactory {
	...
	/* Lazy initialization */
	public static boolean lazyMode;
	public static boolean lazySubtypes;
	public static int lazyLimit;
	...
	private void setupGreen(Config conf) {
	 ...
	 public  SymbolicInstructionFactory (Config conf){
      ...
      startSystemMillis = System.currentTimeMillis();
		...
		String lazy = conf.getProperty("symbolic.lazy");
		lazyMode = (lazy != null && lazy.equals("true"));
		System.out.println("symbolic.lazy=" + lazyMode);
		if (lazyMode) {
			lazySubtypes = conf.getBoolean("symbolic.lazy.subtypes", false);
			lazyLimit = conf.getInt("symbolic.lazy.limit", 9999999);
			System.out.println("symbolic.lazy.subtypes=" + lazySubtypes);
			System.out.println("symbolic.lazy.limit=" + lazyLimit);
		}
		...
		if (intmin != null && intmin[0] != null)
			MinMax.MININT = Integer.valueOf(intmin[0]);
		if (intmax != null && intmax[0] != null)
			MinMax.MAXINT = Integer.valueOf(intmax[0]);
		...
		if (dontcare != null && dontcare[0] != null) {
			SymbolicInteger.UNDEFINED = Integer.valueOf(dontcare[0]);
