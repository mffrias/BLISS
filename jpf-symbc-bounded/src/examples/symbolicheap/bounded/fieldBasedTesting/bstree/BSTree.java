...
public class BSTree implements Serializable {
	...
	public static final BSTree SYMBOLICBSTREE = new BSTree(); //field added to execute the hybridRepOK.
	...
	public boolean hybridRepOK(){
		...
		if (root == Node.SYMBOLICNODE)
			return true;
        if (root == null){
            //return true;
        	if (size == BSTree.SYMBOLICINT)
        		return true;
        	return size == 0;
        }
        ...
        if (root == Node.SYMBOLICNODE || size == BSTree.SYMBOLICINT)
        	return true;
        if (numNodeshybrid(root) != size)
            return false;
        // checks that data is ordered
        if (root == Node.SYMBOLICNODE)
        	return true;
        if (!isOrderedhybrid(root))
            return false; 
        return true;
    }
	
	
    private boolean isAcyclichybrid() {
        ...
        while (!workList.isEmpty()) {
            ...
            if (current == Node.SYMBOLICNODE || current.left == Node.SYMBOLICNODE)
            	return true;
            ...
            if (current == Node.SYMBOLICNODE || current.right == Node.SYMBOLICNODE)
            	return true;
    ...
    private int numNodeshybrid(Node n) {
    	if (n == Node.SYMBOLICNODE)
    		return Node.SYMBOLICINT;
        if (n == null)
            return 0;
        if (n == Node.SYMBOLICNODE || n.left == Node.SYMBOLICNODE || n.right == Node.SYMBOLICNODE)
        	return Node.SYMBOLICINT;
        return 1 + numNodeshybrid(n.left) + numNodeshybrid(n.right);
    }

	
    
    private boolean isOrderedhybrid(Node n) {
    	if (n == Node.SYMBOLICNODE)
    		return true;
        return isOrderedhybrid(n, -1, -1);
    }

    private boolean isOrderedhybrid(Node n, int min, int max) {
        // if (n.key == null)
        // return false;
    	if (n == Node.SYMBOLICNODE || n.key == Node.SYMBOLICINT)
    		return true;
        if (n.key == -1)
            return false;
        ...
        if ((min != -1 && n.key <= (min)) || (max != -1 && n.key >= (max)))
            return false;
        if (n == Node.SYMBOLICNODE || n.left == Node.SYMBOLICNODE)
        	return true;
        if (n.left != null){
        	if (n == Node.SYMBOLICNODE || n.key == Node.SYMBOLICINT)
        		return true;
            if (!isOrderedhybrid(n.left, min, n.key))
                return false;
        }
        if (n == Node.SYMBOLICNODE || n.right == Node.SYMBOLICNODE)
        	return true;        
        if (n.right != null){
           	if (n == Node.SYMBOLICNODE || n.key == Node.SYMBOLICINT)
        		return true;
	 ...
	 * @Ensures return==true <=> (some n: Node | n in this.nodes && n.key == x ) ; 
	 */
	public /*@ pure @*/ boolean findOracle(int x) {
		Node current = root;
...
/*	
	...
	*/
	...
	public static void main(String[] args) {
		...
		BSTree X = new BSTree();
		X = (BSTree) Debug.makeSymbolicRef("X", X);
