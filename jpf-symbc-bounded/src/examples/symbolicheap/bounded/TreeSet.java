...
public class TreeSet {
	...
	public TreeSet addMutated(int aKey, TreeSet root) {
		TreeSet t = root;
		...
		while (boolean_true) {
			...
			if (aKey == t.key) {
				...
				if (t.left != null) {
					...
					root = null;//fixAfterInsertion(t.left, root); 
	...
	//    public static void main(String[] args) {
	//
	//    	TreeSet N0 = new TreeSet(60, BLACK);
	////    	N0 = (TreeSet) Debug.makeSymbolicRef("N0", N0);
	//    	N0 = (TreeSet) Debug.makeSymbolicRefBounded("N0", N0);
	//    	
	////    	TreeSet N1 = new TreeSet(50, BLACK);
	////    	TreeSet N2 = new TreeSet(75, BLACK);
	////    	TreeSet N3 = new TreeSet(90, RED);
	////
	////    	N0.left = N1;
	////    	N0.right = N2;
	////    	N2.right = N3;
	...
	////    	N3.parent = N2;
	//    	
	//    	if (N0 != null && N0.repOK_Concrete(N0)) {
	//    		//    		try {
	//    		N0 = N0.remove(50);
	//    		if (N0 != null)
	//    			N0.countStructure(N0);
	//    		//    		} catch (Exception e) {}
	//    		if (N0 != null && !N0.repOK_ConcretePost(N0)){
	//    			N0.dumpStructure(N0);
	//    		}
	//
	//    	}
	...
	public boolean repOK_ConcreteMutant(TreeSet root) {
		return repOK_StructureMutant(root) && repOK_ColorsMutant(root) && repOK_KeysAndValuesMutant(root);
	}

	public boolean repOK_StructureMutant(TreeSet root) {
		Set<TreeSet> visited = new HashSet<TreeSet>();
		List<TreeSet> worklist = new ArrayList<TreeSet>();
		visited.add(root);
		worklist.add(root);
	...
	public boolean repOK_ColorsMutant(TreeSet root) {
		if (root.color() != BLACK)
			return false;
		List<TreeSet> worklist = new ArrayList<TreeSet>();
		worklist.add(root);
		while (!worklist.isEmpty()) {
			...
			TreeSet cl = current.left();
			TreeSet cr = current.right();
			if (current.color() == BLACK) { //<==== RED mutado a BLACK
				if (cl != null && cl.color() == RED) {
					return false;
				}
				if (cr != null && cr.color() == RED) {
	...
	public boolean repOK_KeysAndValuesMutant(TreeSet root) {
		int min = repOK_findMin(root);
		int max = repOK_findMax(root);
		if (!repOK_orderedKeys(root, min - 1, max + 1)) {
			return false;
