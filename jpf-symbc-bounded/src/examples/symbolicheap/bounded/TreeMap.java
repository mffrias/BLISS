...
//		tree = (TreeMap) DebugBounded.makeSymbolicRef("tree", tree);
        tree = (TreeMap) DebugBounded.makeSymbolicRefBounded("tree", tree);

		try {

			if (tree != null){
				int switchCtr = DebugBounded.makeSymbolicInteger("switchCtr1");

				if (switchCtr == 1) {
					int cont = DebugBounded.makeSymbolicInteger("cont1");
					tree.containsKey(cont);
				} else if (switchCtr == 2) {
					tree.print();
				} else if (switchCtr == 3) {
					int put = DebugBounded.makeSymbolicInteger("put1");
					tree.put(put);
				} else if (switchCtr == 4) {
					int remove = DebugBounded.makeSymbolicInteger("remove1");
					tree.remove(remove);
				} else {
					tree.size();
				}
