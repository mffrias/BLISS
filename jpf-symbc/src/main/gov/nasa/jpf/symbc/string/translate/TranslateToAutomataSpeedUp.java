	...
	public static boolean isSat (StringGraph g, PathCondition pc) {
		...
		while (restart) {
			...
			for (Vertex v: g.getVertices()) {
				mapSolved.put(v, Integer.valueOf(0));
			...
			for (Edge e: g.getEdges()) {
				if (e instanceof EdgeConcat) {
					...
					if (i == null) {
						mapEdgeCount.put(e.getSources().get(0), Integer.valueOf(1));
					}
					else {
						mapEdgeCount.put(e.getSources().get(0), Integer.valueOf(i+1));
					}
					i = mapEdgeCount.get(e.getSources().get(1));
					if (i == null) {
						mapEdgeCount.put(e.getSources().get(1), Integer.valueOf(1));
					}
					else {
						mapEdgeCount.put(e.getSources().get(1), Integer.valueOf(i+1));
					}
					i = mapEdgeCount.get(e.getDest());
					if (i == null) {
						mapEdgeCount.put(e.getDest(), Integer.valueOf(1));
					}
					else {
						mapEdgeCount.put(e.getDest(), Integer.valueOf(i+1));
					}
				}
				else {
					Integer i = mapEdgeCount.get(e.getSource());
					if (i == null) {
						mapEdgeCount.put(e.getSource(), Integer.valueOf(1));
					}
					else {
						mapEdgeCount.put(e.getSource(), Integer.valueOf(i+1));
					}
					i = mapEdgeCount.get(e.getDest());
					if (i == null) {
						mapEdgeCount.put(e.getDest(), Integer.valueOf(1));
